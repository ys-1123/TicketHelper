...
public class TicketMasterClient {
	...
	private static final String API_KEY = "HXllVfwY66mVKq7r4p28ZVGYmi7Q71v5";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			// translate special chars in String keyword
			// to make it acceptable by URL
			// Ex: "New York" -----> "New%20York"
			// Usually this is done by browser, but here the request did
			// not go through browser, this has to be handled by the sender
			keyword = URLEncoder.encode(keyword, "UTF-8");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		// HTTP URL does not allow certain chars
		...
		try {
			...
			// In client's perspective, the response from server is "input"
		...
		try {
			...
			// tell if key "_embedded" exist in object
